...
def isnewuser(username):
    ...
    data = users.getall()
    usernames = [user.get('userSHA') for user in data]
...
def send_money(sender, recipient, amount):
    ...
    #verify that the recipient exists
        ...
        raise InvalidTransactionException("Kullanıcı bulunamadı.", recipient)
...
############################################################################################################
def send_carbon(sender, recipient, amount):
    #verify that the amount is an integer or floating value
    try: amount = float(amount)
    except ValueError:
        raise InvalidTransactionException("Geçersiz işlem.")
    ...
    #verify that the user has enough money to send (exception if it is the BANK)
    if amount > get_Cbalance(sender) and sender != "BANK":
        raise InsufficientFundsException("Yetersiz miktar.", get_Cbalance(sender))
    ...
    blockchain = get_blockchain()
    number = len(blockchain.chain) + 1
    data = "%s-->%s-->%s" %(recipient, sender, RC)
    blockchain.mine(Block(number, data=data))
    sync_blockchain(blockchain)
    #update the blockchain and sync to mysql############################################
    Cblockchain = get_Cblockchain()
    number = len(Cblockchain.chain) + 1
    data = "%s-->%s-->%s" %(sender, recipient, amount)
    Cblockchain.mine(Block(number, data=data))
    sync_Cblockchain(Cblockchain)

def buy_carbon(sender, recipient, amount):
    #verify that the amount is an integer or floating value
    try: amount = float(amount)
    except ValueError:
        raise InvalidTransactionException("Geçersiz işlem.")
...
def get_Cbalance(username):
    balance = 0.00
    blockchain = get_Cblockchain()
...
def get_Cblockchain():
    blockchain = Blockchain()
    blockchain_sql = Table("Cblockchain", "number", "hash", "previous", "data", "nonce")
    for b in blockchain_sql.getall():
        blockchain.add(Block(int(b.get('number')), b.get('previous'), b.get('data'), int(b.get('nonce'))))
...
#update blockchain in mysql table
def sync_Cblockchain(blockchain):
    blockchain_sql = Table("Cblockchain", "number", "hash", "previous", "data", "nonce")
    blockchain_sql.deleteall()
